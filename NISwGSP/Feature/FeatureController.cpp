...
double FeatureDescriptor::getDistance(const FeatureDescriptor& _descriptor1,
    ...
    const vector<cv::Mat>& data1 = _descriptor1.data;
    const vector<cv::Mat>& data2 = _descriptor2.data;
...
void FeatureController::detect(const cv::Mat& _grey_img,
    ...
    if (vl_sift_process_first_octave(vlSift, (vl_sift_pix const*)grey_img_float.data) != VL_ERR_EOF) {
        do {
            ...
            for (int i = 0; i < vlSift->nkeys; ++i) {
                double angles[4];
                _feature_points.emplace_back(vlSift->keys[i].x*2+offsetx , vlSift->keys[i].y*2+offsety);
