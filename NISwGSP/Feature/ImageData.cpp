...
void ImageData::update_image(string _img_sequence){
    img_sequence = _img_sequence;
    cout << *file_dir + _img_sequence + "/" + file_name + file_extension << endl;
    img = cv::imread(*file_dir + _img_sequence + "/" + file_name + file_extension);
    rgba_img = cv::imread(*file_dir + _img_sequence + "/" + file_name + file_extension, cv::IMREAD_UNCHANGED);

    float original_img_size = img.rows * img.cols;
    feature_height = img.rows/4;

    if (original_img_size > DOWN_SAMPLE_IMAGE_SIZE.width * DOWN_SAMPLE_IMAGE_SIZE.height) {
        //float scale = sqrt(DOWN_SAMPLE_IMAGE_SIZE / original_img_size);
        resize(img, img, DOWN_SAMPLE_IMAGE_SIZE);
        resize(rgba_img, rgba_img, DOWN_SAMPLE_IMAGE_SIZE);
        feature_height = img.rows / 4;
    }
...
const cv::Mat& ImageData::getGreyImage() const {
    ...
    resize(grey_img, grey_img, cv::Size(grey_img.cols / 2, grey_img.rows / 2));

    grey_imgL = grey_img(cv::Rect(0, grey_img.rows/4, grey_img.cols, grey_img.rows/2 ));
    grey_imgR = grey_img(cv::Rect(0, grey_img.rows/4, grey_img.cols, grey_img.rows/2 ));
...
const vector<Point2>& ImageData::getFeaturePoints(const enum REGION& region) const {
    if (feature_points.empty()) {
        getGreyImage();
        if (region == LEFT) {
            FeatureController::detect(grey_imgL, feature_points, feature_descriptors, 0,feature_height);
        }
        else {
            FeatureController::detect(grey_imgR, feature_points, feature_descriptors, 0,feature_height);
        }
    }
...
const vector<FeatureDescriptor>& ImageData::getFeatureDescriptors(const enum REGION& region) const {
