...
class ImageData {
public:
    string file_name,img_sequence, file_extension;
    void update_image(string _img_sequence);
    ...
    const vector<Point2>& getFeaturePoints(const enum REGION& region) const;
    const vector<FeatureDescriptor>& getFeatureDescriptors(const enum REGION& region) const;

    void clear();

    cv::Mat img, rgba_img, alpha_mask;
...
private:
    ...
    int feature_width;
    int feature_height;
    mutable cv::Mat grey_img;
    mutable cv::Mat grey_imgL;
    mutable cv::Mat grey_imgR;
