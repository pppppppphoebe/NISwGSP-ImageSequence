    ...
    if(result.empty()) {
        ...
        switch (_global_rotation_method) {
            case GLOBAL_ROTATION_2D_METHOD:
            {
               ...
               for(int i = 0; i < images_data.size(); ++i) {
                    result[i].theta = roundf(atan2(x[DIMENSION_2D * i + 1], x[DIMENSION_2D * i]) * 100) / 100;
               }
            }
                break;
...
const vector<vector<vector<pair<int, int> > > > & MultiImages::getFeaturePairs() const {
    if(feature_pairs.empty()) {
        ...
        for(int i = 0; i < images_match_graph_pair_list.size(); ++i) {
            ...
            const vector<Point2> & m1_fpts = images_data[match_pair.first ].getFeaturePoints(RIGHT);
            const vector<Point2> & m2_fpts = images_data[match_pair.second].getFeaturePoints(LEFT);
            vector<Point2> X, Y;
...
const vector<vector<vector<Point2> > > & MultiImages::getFeatureMatches() const {
    if(feature_matches.empty()) {
        ...
        for(int i = 0; i < images_match_graph_pair_list.size(); ++i) {
            ...
            const vector<Point2> & m1_fpts = images_data[m1].getFeaturePoints(RIGHT);
            const vector<Point2> & m2_fpts = images_data[m2].getFeaturePoints(LEFT);
...
vector<pair<int, int> > MultiImages::getInitialFeaturePairs(const pair<int, int> & _match_pair) const {
    ...
    const int feature_size_1 = (int)images_data[_match_pair.first ].getFeaturePoints(RIGHT).size();
    const int feature_size_2 = (int)images_data[_match_pair.second].getFeaturePoints(LEFT).size();
    ...
    for(int p = 0; p < pair_count; ++p) {
        ...
        const vector<FeatureDescriptor> & feature_descriptors_1 = images_data[pair_match[ p]].getFeatureDescriptors(RIGHT);
        const vector<FeatureDescriptor> & feature_descriptors_2 = images_data[pair_match[!p]].getFeatureDescriptors(LEFT);
...
void MultiImages::meshOptimize2(vector<vector<Point2> >& _vertices, Size2& _target_size) {
    
    vector<Rect_<FLOAT_TYPE> > rects = getVerticesRects<FLOAT_TYPE>(_vertices);

    int lastindex = rects.size() - 1;

    vector<WarpingMesh> warpingmesh;
    for (int i = 0; i < rects.size(); i++) {
        WarpingMesh mesh = WarpingMesh(images_data[0].mesh_2d->nw, images_data[0].mesh_2d->nh, _vertices[i]);
        warpingmesh.push_back(mesh);
    }

    int vertex_nw = images_data[0].mesh_2d->nw + 1;
    int vertex_nh = images_data[0].mesh_2d->nh + 1;

    int mesh_centerX_idx1 = 0;
    int mesh_centerY_idx1 = (vertex_nh - 1) / 2;


    int mesh_centerX_idx2 = (vertex_nw - 1);
    int mesh_centerY_idx2 = (vertex_nh - 1) / 2;

    //find first and last mesh avg height;
    float minheight_ = _vertices[0][mesh_centerX_idx1].y;
    float maxheight_ = _vertices[0][(vertex_nh - 1) * (vertex_nw ) + mesh_centerX_idx1].y;

    float minheight2_ = _vertices[lastindex][mesh_centerX_idx2].y;
    float maxheight2_ = _vertices[lastindex][(vertex_nh - 1) * (vertex_nw ) + mesh_centerX_idx2].y;

    float minheightavg = (minheight_ + minheight2_) / 2;
    float maxheightavg = (maxheight_ + maxheight2_) / 2;

    vector<float> firstMeshVertexY_value(vertex_nw * vertex_nh, 0);
    vector<float> lastMeshVertexY_value(vertex_nw * vertex_nh, 0);

    cout << "average min height: " << minheightavg << " average max height: " << maxheightavg << endl;

    int center_idx1 = (mesh_centerY_idx1)*vertex_nw + mesh_centerX_idx1;
    int center_idx2 = (mesh_centerY_idx2)*vertex_nw + mesh_centerX_idx2;

    vector<Point2> meshVertices = images_data[lastindex].mesh_2d->getVertices();
    Point2 origin(rects[lastindex].x, rects[lastindex].y);

    float width = images_data[0].mesh_2d->nw * images_data[0].mesh_2d->lw;
    float height = images_data[0].mesh_2d->nh * images_data[0].mesh_2d->lh;

    //optimize first and last (1st,5th) mesh 
    for (int yid = 0; yid < vertex_nh; yid++) {
        for (int xid = 0; xid < vertex_nw; xid++) {

            int vertexid = yid * vertex_nw + xid;

            if (xid == 0) {
                float targety = meshVertices[vertexid].y / height * (maxheightavg - minheightavg) + minheightavg;

                //optimize first mesh middle 
                float y_dif = (targety - _vertices[0][vertexid].y);
                firstMeshVertexY_value[vertexid] = y_dif;
            }
            if (xid == vertex_nh - 1) {
                float targety = meshVertices[vertexid].y / height * (maxheightavg - minheightavg) + minheightavg;

                //optimize last mesh middle
                float y_dif = (targety - _vertices[lastindex][vertexid].y);
                lastMeshVertexY_value[vertexid] = y_dif;
            }
                ...
                //optimize first mesh
                if (_vertices[0][vertexid].x > _vertices[0][center_idx1].x)
                    weight = 1 - (_vertices[0][vertexid].x - _vertices[0][center_idx1].x) / (rects[0].width - _vertices[0][center_idx1].x);
                else
                    weight = 1;
                float y_dif = (targety - _vertices[0][vertexid].y);
                firstMeshVertexY_value[vertexid] = y_dif * weight;


                //optimize last mesh
                if (_vertices[lastindex][vertexid].x < _vertices[lastindex][center_idx2].x)
                    weight = (_vertices[lastindex][vertexid].x - rects[lastindex].x) / (_vertices[lastindex][center_idx2].x - rects[lastindex].x);
                else
                    weight = 1;
                y_dif = (targety - _vertices[lastindex][vertexid].y);
                lastMeshVertexY_value[vertexid] = y_dif * weight;
            }
        }
    }


    //optimize neighbour (2nd,4th) mesh and set final vertices;
    for (int yid = 0; yid < vertex_nh; yid++) {
        for (int xid = 0; xid < vertex_nw; xid++) {
            int vertex_id;
            int quad_id;
            ...
            //optimize 2nd mesh;
            vertex_id = (yid)*vertex_nw + xid;
            pt = _vertices[1][vertex_id];
            quad_id = warpingmesh[0].findPointNearestMeshQuadIndex(pt);

            if (quad_id != -1) {
                vector<float> weights = warpingmesh[0].getInterpolateVertexWeights(pt, quad_id);

                int gridy = quad_id / images_data[0].mesh_2d->nw;
                int gridx = quad_id % images_data[0].mesh_2d->nw;
                int vertexY_value = 0;

                for (int n = 0; n < weights.size(); n++) {
                    int v_id = (gridy + nexts[n].y) * vertex_nw + (gridx + nexts[n].x);
                    vertexY_value += weights[n] * firstMeshVertexY_value[v_id];
                }

                Point2 newP(_vertices[1][vertex_id].x, _vertices[1][vertex_id].y + vertexY_value);
                _vertices[1][vertex_id] = newP;

            }

            // optimize 4th mesh;
            vertex_id = (yid)*vertex_nw + xid;
            pt = _vertices[lastindex - 1][vertex_id];
            quad_id = warpingmesh[lastindex].findPointNearestMeshQuadIndex(pt);

            if (quad_id != -1) {
                vector<float> weights = warpingmesh[lastindex].getInterpolateVertexWeights(pt, quad_id);
                ...
                for (int n = 0; n < weights.size(); n++) {
                    int v_id = (gridy + nexts[n].y) * vertex_nw + (gridx + nexts[n].x);
                    vertexY_value += weights[n] * lastMeshVertexY_value[v_id];
                }

                Point2 newP(_vertices[lastindex - 1][vertex_id].x, _vertices[lastindex - 1][vertex_id].y + vertexY_value);
                _vertices[lastindex - 1][vertex_id] = newP;
            }
        }
    }

    //set first and last mesh vertices;
    for (int yid = 0; yid < vertex_nh; yid++) {
        for (int xid = 0; xid < vertex_nw; xid++) {

            //set first and last mesh's middle vertices
            if (xid == 0 ) {
                int vertex_id = (yid)*vertex_nw + xid;

                Point2 newP(_vertices[0][center_idx1].x, _vertices[0][vertex_id].y + firstMeshVertexY_value[vertex_id]);
                _vertices[0][vertex_id] = newP;
            }
           
            else {
                int vertex_id = (yid)*vertex_nw + xid;

                //set first mesh vertices
                Point2 newP(_vertices[0][vertex_id].x, _vertices[0][vertex_id].y + firstMeshVertexY_value[vertex_id]);
                _vertices[0][vertex_id] = newP;
            }
        }
    }
    ...
    for (int yid = 0; yid < vertex_nh; yid++) {
        for (int xid = 0; xid < vertex_nw; xid++) {
           if (xid == vertex_nw - 1) {
             int vertex_id = (yid)*vertex_nw + xid;
             Point2  newP = Point2(_vertices[lastindex][center_idx2].x, _vertices[lastindex][vertex_id].y + lastMeshVertexY_value[vertex_id]);
             _vertices[lastindex][vertex_id] = newP;
           }
           else {
               int vertex_id = (yid)*vertex_nw + xid;

               Point2 newP(_vertices[lastindex][vertex_id].x, _vertices[lastindex][vertex_id].y + lastMeshVertexY_value[vertex_id]);
               _vertices[lastindex][vertex_id] = newP;
           }
        }
    }
    //define panorama cropping size
    //define cropping height range
    float min_y = -MAXFLOAT;
    float max_y = MAXFLOAT;
    for (int i = 0; i < rects.size(); ++i) {
        int w = images_data[i].mesh_2d->nw + 1;
        int h = images_data[i].mesh_2d->nh + 1;

        for (int xid = 0; xid < w; xid++) {
            int vertexid = xid;
            if (min_y < _vertices[i][vertexid].y) {
                min_y = _vertices[i][vertexid].y;
            }
            vertexid = (h - 1) * w + xid;
            if (max_y > _vertices[i][vertexid].y) {
                max_y = _vertices[i][vertexid].y;
            }
        }
    }
    int crop_size;
    ...
    crop_start_y = min_y;
    crop_end_y = _target_size.height - max_y;

    //define cropping width range
    crop_start = _vertices[0][center_idx1].x;
    crop_end = _target_size.width -(_vertices[lastindex][center_idx2].x);

    const int& center_index = parameter.center_image_index;
    int vertexid = (vertex_nh / 2) * vertex_nw + vertex_nw / 2;
}


void MultiImages::updateImages(int id) {
    cur_seq = img_sequence[id];
    for (int i = 0; i < images_data.size(); i++) {
        images_data[i].update_image(cur_seq);
    }
    return;
}


void MultiImages::textureMapping(vector<vector<Point2> > & _vertices) const {
    ...
    for (int i = 0; i < images_data.size(); ++i) {
        ...
        Mat polygon_mask(rects[i].height + shift.y, rects[i].width + shift.x, CV_32SC1, Scalar::all(NO_GRID));
        ...
        for(int j = 0; j < polygons_indices.size(); ++j) {
            for(int k = 0; k < images_data[i].mesh_2d->getTriangulationIndices().size(); ++k) {
                ...
                fillConvexPoly(polygon_mask, contour, TRIANGLE_COUNT, label, LINE_AA, PRECISION);
                Point2f src[] = {
        ...
        All_affine_transforms.emplace_back(affine_transforms);
        polygon_index_masks.emplace_back(polygon_mask);
    }
}

void MultiImages::writeWarpingImages(vector<vector<Point2> >& _vertices, Size2& _target_size) {
    ...
    for (int i = 0; i < images_data.size(); ++i) {
        ...
        Mat polygon_mask = polygon_index_masks[i];
        vector<Mat> affine_transforms = All_affine_transforms[i];

        for (int y = 0; y < image.rows; ++y) {
            for (int x = 0; x < image.cols; ++x) {
                int polygon_index = polygon_mask.at<int>(y, x);
                ...
                if (polygon_index != NO_GRID) {
                    ...
                    if (p_f.x >= 0 && p_f.y >= 0 &&
                        ...
                        if (i == 0 || i == images_data.size() - 1) {
                            if (i == 0) {
                                if (i == 0 && x >= (int)image.cols / 2) {
                                    image.at<Vec4b>(y, x) = Vec4b(c[0], c[1], c[2], alpha[0]);
                                    //image.at<Vec4b>(y, x) = Vec4b(0, 0, 0, 0);
                                    /*if (x < min_x) {
                                        min_x = x;
                                    }*/
                                }
                            ...
                            if (i == images_data.size() - 1) {
                                if (i == images_data.size() - 1 && x < (int)image.cols / 2) {
                                    image.at<Vec4b>(y, x) = Vec4b(c[0], c[1], c[2], alpha[0]);
                                    //image.at<Vec4b>(y, x) = Vec4b(0, 0, 0, 0);
                                   /* if (x > max_x) {
                                        max_x = x;
    ...
    for (int i = 0; i < images_data.size(); ++i) {
        ...
        for (int y = 0; y < warp_images[i].rows; ++y) {
            for (int x = 0; x < warp_images[i].cols; ++x) {

                cv::Point2i p_source(x, y);
                cv::Point2i p_target(round(p_source.x + origins[i].x), round(p_source.y + origins[i].y));
                if (p_target.x >= 0 && p_target.x < result.cols &&
                    p_target.y >= 0 && p_target.y < result.rows) {
                    cv::Vec4b v = warp_images[i].at<cv::Vec4b>(p_source);
                    cv::Vec3f value = cv::Vec3f(v[0], v[1], v[2]);
                    result.at<cv::Vec4b>(p_target) = v;
                }
            }
        }
        ...
        if (debugMode) {
            ...
            const Mat warpMesh(result.size() + Size(line_thickness * 6, line_thickness * 6), CV_8UC4);
            const Point2 shift(line_thickness * 3, line_thickness * 3);
            const Rect rect(shift, result.size());

            result.copyTo(warpMesh(rect));
            ...
            Mat image = warpMesh(rect);
            
            Mat cropedImage = image(Rect(crop_start, 0, image.cols - crop_start - crop_end, image.rows));

            resize(cropedImage, cropedImage,Size(1500,750));
            imwrite(parameter.debug_dir + cur_seq +"_" + std::to_string(i) + "warpmesh.png", image);
        }

        Mat cropedImage = result(Rect(crop_start, crop_start_y, _target_size.width - (crop_start)-(crop_end+1), _target_size.height - crop_start_y - crop_end_y));
        imwrite(parameter.result_dir + cur_seq + "/" + std::to_string(i) + ".png", cropedImage);
    }
}

Mat MultiImages::writeResult(vector<vector<Point2> >& _vertices, Size2& _target_size, const string& _postfix, const BLENDING_METHODS& _blend_method) {
                    ...
                    if (p_f.x >= 0 && p_f.y >= 0 &&
                        ...
                        if (i == 0 || i == images_data.size() - 1) {
                            if (i == 0) {
                                if (i == 0 && x >= (int)image.cols / 2) {
                                    image.at<Vec4b>(y, x) = Vec4b(c[0], c[1], c[2], alpha[0]);
                                    //image.at<Vec4b>(y, x) = Vec4b(255, 255, 255, alpha[0]);
                                }
                                else {
                                    image.at<Vec4b>(y, x) = Vec4b(0,0,0,0);
                                }
    ...
    cv::Mat result = Blending(warp_images, origins, _target_size, new_weight_mask, _blend_method == BLEND_AVERAGE);
    Mat cropedImage = result(Rect(crop_start, crop_start_y, _target_size.width - (crop_start)-(crop_end+1), _target_size.height - crop_start_y - crop_end_y));
    imwrite(parameter.result_dir + _postfix + ".png", cropedImage);
    return  result;
}
...
void MultiImages::writeResultWithMesh(const Mat & _result,
    ...
    Mat image = result(rect);
    Mat cropedImage = image(Rect(crop_start, 0, image.cols - crop_start - crop_end, image.rows));
    imwrite(parameter.debug_dir + parameter.file_name + _postfix + ".png", cropedImage);
}

void MultiImages::writeMiddleLine(const Mat& _result,
    const vector<vector<Point2> >& _vertices,
    const string& _postfix,
    const bool _only_border) const {
    ...
    for (int i = 0; i < images_data.size(); ++i) {
        vector<Point2> meshVertices = images_data[i].mesh_2d->getVertices();
        ...
        int yid = vertex_nh / 2;
        for (int xid = 0; xid < vertex_nw; xid++) {
            int vertexid = yid * vertex_nh + xid;
            const Scalar& color = getBlueToRedScalar((2. * i / (images_data.size() - 1)) - 1) * 255;
            if (xid + 1 < vertex_nw) {
                line(result,
                    _vertices[i][vertexid] + shift,
                    _vertices[i][vertexid + 1] + shift, color, line_thickness, LINE_8);
            }
    ...
    Mat cropedImage = image(Rect(crop_start, crop_start_y, image.cols - crop_start - crop_end, image.rows -crop_start_y - crop_end_y));
    imwrite(parameter.debug_dir + parameter.file_name + _postfix + ".png", cropedImage);
}
...
void MultiImages::writeImageOfFeaturePairs(const string & _name,
    ...
    const vector<Point2> & m1_fpts = images_data[_match_pair.first ].getFeaturePoints(RIGHT);
    const vector<Point2> & m2_fpts = images_data[_match_pair.second].getFeaturePoints(LEFT);
    vector<Point2> f1, f2;
