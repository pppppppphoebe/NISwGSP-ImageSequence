...
public:
    ...
    void updateImages(int id);

    void meshOptimize(vector<vector<Point2> >& _vertices, Size2& _target_size);


    void meshOptimize2(vector<vector<Point2> >& _vertices, Size2& _target_size);

    FLOAT_TYPE getImagesMinimumLineDistortionRotation(const int _from, const int _to) const;
    
    void writeWarpingImages(vector<vector<Point2> >& _vertices, Size2& _target_size);
    
    void textureMapping(vector<vector<Point2> > & _vertices) const;

    Mat writeResult(vector<vector<Point2> >& _vertices, Size2& _target_size, const string& _postfix, const BLENDING_METHODS& _blend_method);
    ...
    void writeMiddleLine(const Mat& _result,
        const vector<vector<Point2> >& _vertices,
        const string& _postfix,
        const bool _only_border) const;
...
private:    
    ...
    mutable vector<vector<Mat>> All_affine_transforms;
    mutable vector<Mat> polygon_index_masks;
};
