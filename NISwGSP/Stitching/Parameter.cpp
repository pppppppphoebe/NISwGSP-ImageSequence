...
Parameter::Parameter(const string & _file_name) {
    ...
    string folder_path = "./input-42-data/" +  _file_name;
    ...
    for (int i = 0; i < img_sequence.size(); i++) {
        _mkdir((result_dir+img_sequence[i]).c_str());
    }
...
//    mkdir(debug_dir.c_str(), S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    ...
    image_file_full_names = getImageFileFullNamesInDir(file_dir+ "/" + img_sequence[0]+"/");
    cout << file_dir + "/" + img_sequence[0] + "/" << endl;
    /*** configure ***/
    grid_size = GRID_SIZE;
    down_sample_image_size = DOWN_SAMPLE_IMAGE_SIZE.width* DOWN_SAMPLE_IMAGE_SIZE.height;
